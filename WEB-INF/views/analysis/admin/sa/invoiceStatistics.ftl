            ...
            <div class="row">
                <div class="control-group">
                    <div class="controls control-row-auto ml0">
                        <label class="control-label control-label-auto mr30">发货订单量：
                            <span class="red" id="orderQuantity">${(invoiceStatistics.orderQuantity)!0}</span>
                        </label>
                        <label class="control-label control-label-auto mr30">物流费用总额：
                            <span class="red" id="orderTotalAmt">￥${(invoiceStatistics.expressTotalAmt)!0.00}</span>
                        </label>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function generateAllReport() {
        location.href = "${ctx}/admin/sa/invoiceStatistics/exportInvoiceStatistics";
...
<script type="text/javascript">
    ...
    BUI.use(['common/search', 'common/page'], function (Search) {
        var columns = [
            ...
            {title: '发货时间', dataIndex: 'sendTime', width: 160, renderer: BUI.Grid.Format.datetimeRenderer},
            {title: '操作人', dataIndex: 'operUserName', width: 120},
            {title: '配送方式', dataIndex: 'expressName', width: 200},
            {title: '物流单号', dataIndex: 'orderExpressNum', width: 200},
            {
                title: '物流费用', dataIndex: 'orderExpressAmt', width: 160, renderer: function (value, rowObj) {
                return value.toFixed(2);
            }
            },
            {title: '收货人', dataIndex: 'receiveName', width: 160},
            {title: '会员', dataIndex: 'userLoginName', width: 200}
        ];

        var store = Search.createStore('${ctx}/admin/sa/invoiceStatistics/grid_json', {pageSize: 10});
