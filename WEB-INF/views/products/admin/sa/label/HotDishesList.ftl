        ...
        <div id="tab">
            <ul class="bui-tab nav-tabs" aria-disabled="false" aria-pressed="false">
                <li class="bui-tab-item bui-tab-item-selected" aria-disabled="false" aria-pressed="false">
                    <span class="bui-tab-item-text">热销菜品</span>
                </li>
            </ul>
        </div>
    </div>
    ...
    <div class="content-top">
        <div class="row">
            <div class="control-group span20">
                设置热销推荐菜品  最多只能设置20款，会在商城相关菜品页面与操作之后推荐给用户
            </div>
        </div>
    </div>
...
BUI.use(['common/search','common/page'],function (Search) {
    var hotProductColumns = [
        ...
        {title:'菜品名称',dataIndex:'productName',width:150, renderer : function(value, rowObj){
        ...
        {title:'操作',dataIndex:'',width:200,renderer : function(value,rowObj){
            var strSort= "";
                ...
                strSort+='<a href="${ctx}/admin/sa/label/deleteHotDishesLabel?hotProductId='+rowObj.productId+'" >移除</a>&nbsp;';
            return strSort;
        }}
    ];

    var hotProductStore = Search.createStore('${ctx}/admin/sa/label/grid_json2',{
        pageSize : 20
    });
    ...
    var hotGridCfg = Search.createGridCfg(hotProductColumns,{
        width: '100%',
            tbar : {
                items : [
                    {text : '选择热销菜品',btnCls : 'button button-small button-primary',handler:addFunction}
                ]
              },
    ...
    function addFunction(){
        ...
        dialog = new Overlay.Dialog({
            ...
            height: 460,
            loader : {
                url : '${ctx}/admin/sa/promotion/productDialog/listForHotProduct?productTypeCd=2',
            ...
            buttons:[{
                ...
                handler : function(){
                    productChoiceEvent(top.getSelectedRecordFoods());
                    this.close();
                }
            }],
            mask:true,
    ...
    function productChoiceEvent(selectedProduct){
        ...
        $.ajax({
            url: "${ctx}/admin/sa/label/setHotDishes",
            data: data,
            type:"post",
            dataType: "text",
            success: function(data){
                if(data=="success"){
                    window.location.href="${ctx}/admin/sa/label/HotDishesList";
                } else {
