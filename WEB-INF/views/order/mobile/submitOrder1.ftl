...
<div id="page" v-cloak>
    ...
    <div class="mui-content">
        <div class="exchangetab">
            <ul>
                <li class="selected"><a href="/m/account/order/submitOrder">快递配送</a></li>
                <li><a href="/m/account/order/submitOrderTake">上门自提</a></li>
            </ul>
        </div>
    ...
    <div class="order-address">
        ...
        <span class="name">林淼淼</span>
        <span class="phone">15900000123</span>
        <address>福建省  厦门市  思明区莲前街道软件园二期</address>
        </a>
    </div>
    -->

        <div class="order-address">
            <a href="/m/account/userAddress/selectAddressList?fromType=1">
                <address v-if="!result.address">您的收货信息为空，点此添加收货地址</address>
                <span v-if="result.address" v-cloak>
                    <span class="name">{{result.address.receiverName}}</span>
                    <span class="phone">{{result.address.receiverTel}}</span>
                    <address>{{result.address.addressFullName}}</address>
                </span>
            </a>
        </div>
                    ...
                    <li class="itemlink">
                        ...
                        <div class="s">
                            <select v-model="form.expressId">
                                ...
                                <option v-for="express in result.expressList" :value="express.expressId">{{express.expressName}}￥{{express.orderExpressAmt}}</option>
                            </select>
                        </div>
                    </li>
                    <li class="itemlink" v-if="conditionTwoIsUse == 1">
                        <div class="c">配送时间</div>
                        <div class="s">
                            <select v-model="form.expectSendTime">
                                <option value="-1">请选择</option>
                                <option v-for="(value, key) in result.expectSendTimeMap" :value="key">{{value}}</option>
                            </select>
                        </div>
                    </li>
                    <li v-if="conditionOneIsUse == 1">
                        <a class="itemlink" href="/m/account/invoice/chooseInvoiceList?type=1">
        ...
        <div id="J_ASSpec" class="actionsheet-spec">
            ...
            <div class="ye-payment" v-show="!showPayType">
                ...
                <div class="paymentwrap">
                    ...
                    <div class="orderinfo" v-show="!result.isPayPassword">
                        <p>您还未设置支付密码，为保障账户资金安全，<br>请先<a href="/m/account/accountSecurity/changePaw_Reset?type=2&successUrl=/m/account/order/submitOrder" class="textcolor">设置支付密码</a>！</p>
                    </div>
                </div>
...
<script>
    var vm = new Vue({
        ...
        watch : {
            'form.expressId': {
                handler: function (val) {
                    if (val != -1) {
                        setCookie('order_express_id', val);
                    }
                }
            }
        },
        computed: {
            ...
            <#--运费-->
            expressAmt : function () {
            	if(!this.isFreight) { return 0; }//包邮
            	if(!this.result.expressList) { return 0; }
            	for(var i = 0; i < this.result.expressList.length; i++) {
            		if(this.result.expressList[i].expressId == this.form.expressId) {
            			return this.result.expressList[i].orderExpressAmt;
            		}
            	}
            	return 0;
            },
        ...
        methods: {
        	...
        	testForm : function () {
		        ...
		        if(this.conditionTwoIsUse == 1 && this.conditionTwoIsRequired ==1 && this.form.expectSendTime == -1) {
		        	mui.toast('请选择配送时间！');
		            return false;
		        }
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.result == 'success') {
                        ...
                        <#--初始化地址Id-->
                        if(result.address) {
                        	vm.form.addrId = result.address.addrId;
                        }
                        <#--初始化选择的配送方式-->
    ...
    if (getCookie('orderId')) {
        window.location.href = "${ctx}/spa/#/m/account/order/detail/" + getCookie('orderId');
    }
    $(function () {
		<#--使用余额-->
