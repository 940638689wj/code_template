    ...
    <div class="center-layout">
        ...
        <div class="order-state">
            <div class="state-lcol">
                ...
                <div :if="@orderHeaderDTO.type ==  3">
                    <div class="state-btns">
                        <a href="javascript:void(0);" class="v-btn" :click="confirmReceive">确认收货</a>
                    </div>
                </div>
                <div :if="@orderHeaderDTO.type ==  4">
                    <div class="state-btns">
                        <a :attr="{href: '/account/order/toReview?orderId=' + orderId}" class="v-btn">评价</a>
                    </div>
                </div>
                <div :if="@orderHeaderDTO.type ==  6">
                    <div class="state-btns">
                        <a href="javascript:void(0);" class="v-btn" :click="delOrder">删除订单</a>
                    </div>
                </div>
            </div>
        ...
        <div class="order-info-mod">
            <div class="dl">
                ...
                <div class="dd">
                    ...
                    <p><span>期望送达时间：</span><em>{{@orderHeaderDTO.expectSendTime}}</em></p>
                </div>
            </div>
            ...
            <div class="dl">
                ...
                <div class="dd">
                    ...
                    <p :if="@orderReceiveInfo.invoiceTypeCd == 1"><span>个人姓名：</span><em>{{@orderReceiveInfo.companyName}}</em>
                    </p>
                    <p :if="@orderReceiveInfo.invoiceTypeCd == 2"><span>单位名称：</span><em>{{@orderReceiveInfo.companyName}}</em>
                    </p>
                </div>
            </div>
        </div>
                ...
                <tr :for="orderItem in @orderHeaderDTO.orderItemList">
                    ...
                    <td>
                        <div :if="orderHeaderDTO.type == 2 || orderHeaderDTO.type == 3">
                            <a :attr="{href: '/account/order/toReturn?orderId='+ orderHeaderDTO.orderId +'&orderItemId=' + orderItem.orderItemId}" class="v-button" :if="!orderItem.applyStatusCd">
                                退款/退货
                            </a>
                            <a :attr="{href: '/account/order/toReturnDetail?orderItemId=' + orderItem.orderItemId}" class="v-button" :if="orderItem.applyStatusCd && (orderItem.applyStatusCd != 2 || orderItem.applyTypeCd != 2)">
                                退款/退货状态：{{orderItem.applyStatusName}}
                            </a>
                            <a :attr="{href: '/account/order/toReturnLogistics?orderId='+ orderHeaderDTO.orderId +'&orderItemId=' + orderItem.orderItemId}" class="v-button" :if="orderItem.applyStatusCd == 2 && orderItem.applyTypeCd == 2">
                                退款/退货状态：{{orderItem.applyStatusName}}
                            </a>
                        </div>
                    </td>
