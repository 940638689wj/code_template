...
<div id="page">
    ...
    <div class="mui-content">
        <div class="skutabbar">
            <ul>
                <li <#if type==1>class="selected"</#if>><a href="${ctx}/m/account/userPromotion/toRedPacket?type=1">未使用</a></li>
                <li <#if type==2>class="selected"</#if>><a href="${ctx}/m/account/userPromotion/toRedPacket?type=2">已使用</a></li>
                <li <#if type==3>class="selected"</#if>><a href="${ctx}/m/account/userPromotion/toRedPacket?type=3">已过期</a></li>
            </ul>
        </div>
    ...
    function pullupRefresh() {
        ...
        $.ajax({
            ...
            data: {
                ...
                pageSize: pageSize,
                type: type,
                promotionType : 1   //1：红包 2：优惠券
            },
            dataType: "json",
            success: function (data) {
                if(data.result=="true"){
                    var list = data.promotionDTOList;
                    var table = document.body.querySelector('.packetslist ul');
                    ...
                    for (var i = 0 ; i < list.length ; i++) {
                        ...
                        li.innerHTML = '<li class="'
                                        ...
                                        + '<div class="hd"></div>'
                                        + '<div class="bd">'
                                        + '<div class="tit"><span>￥<i>'
                                        + list[i].couponCodeValue
                                        + '</i></span><p>'
