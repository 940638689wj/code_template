...
<body>
	<div id="page">
		<#if showTitle?? && showTitle>
			<header class="mui-bar mui-bar-nav">
				...
				<h1 class="mui-title">首单收益</h1>
				<a class="mui-icon"></a>
			</header>
        </#if>

		<div class="mui-content">
			<div class="balance-top" style="background-color: #00C8C8">
				<div class="fl">
					<h2>
						首单收益<span>(元)</span>
					</h2>
					<p>${sumFirstOrderReward!0.00}</p>
				</div>
				<div class="fr" style="width: 80px;">
					<img alt="" src="${staticResourcePath}/images/earnings04.png">
				</div>
			</div>
	...
	<script>
		...
		function loadReward(pageNo){
	        ...
	        var data = {};
	        data.pageNo = pageNo;
	        $.get('${ctx}/m/account/mstoreUser/rewardPage', data, function(resultData){
	            if(resultData){
	    ...
	    function renderUlData(resultData) {
	        for (var i = 0; i < resultData.length; i++) {
		        var rewardAmt = resultData[i].rewardAmt;
