...
<body>
    <div id="page">
        ...
        <div class="mui-content">
            <div class="balance-top hbHeight" style="background-image:url(${staticPath}/images/red_envelope_bg.png);">
                <h3>红包<span>(元)</span></h3>
                <p>${(userConsumeCalc.redPacketBalance?string.currency)!'￥0.00'}</p>
                <a class="withdrawalBtn" href="${ctx}/m/account/withdrawal/toWithdrawalApplication?price=${(userConsumeCalc.redPacketBalance)!}&type=0">提现</a>
            </div>
            <div class="tabbar">
                <ul>
                    <li <#if type=='0'>class="selected" </#if> ><a href="${ctx}/m/account/toRedEnvelope?type=0" >全部</a></li>
                    <li <#if type=='1'>class="selected" </#if> ><a href="${ctx}/m/account/toRedEnvelope?type=1" >收入</a></li>
                    <li <#if type=='2'>class="selected" </#if> ><a href="${ctx}/m/account/toRedEnvelope?type=2" >支出</a></li>
...
<script>
		...
		function loadRedEnvelope(pageNo,type){
	
	        var data = {};
	        data.sort = "time_l desc";	
	        data.pageNo = pageNo;
	        data.type = type;
	        $.get('${ctx}/m/account/redEnvelopePage', data, function(resultData){
	            if(resultData){
	    ...
	    function renderUlData(resultData) {
	        ...
	        for (var i = 0; i < resultData.length; i++) {
		        var redPacketDetailId = resultData[i].redPacketDetailId;
		        var packetIncome = resultData[i].packetIncome;
		        var packetExpend = resultData[i].packetExpend;
				...
				if(type==0){
					if(packetIncome){
						li +='<div class="r">+'+packetIncome+'</div>';
					}
					if(packetExpend){
						li +='<div class="r">-'+packetExpend+'</div>';
					}
