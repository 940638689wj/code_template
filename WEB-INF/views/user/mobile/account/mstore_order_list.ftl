...
<body>
    <div id="page">
        ...
        <div class="mui-content">
            <div class="tabbar">
                <ul>
                	<li <#if type=='0'>class="selected"</#if> ><a href="${ctx}/m/account/mstoreUser/toMstoreOrderList?type=0">全部</a></li>
                    <li <#if type=='1'>class="selected"</#if> ><a href="${ctx}/m/account/mstoreUser/toMstoreOrderList?type=1">待付款</a></li>
                    <li <#if type=='2'>class="selected"</#if> ><a href="${ctx}/m/account/mstoreUser/toMstoreOrderList?type=2">待发货</a></li>
                    <li <#if type=='3'>class="selected"</#if> ><a href="${ctx}/m/account/mstoreUser/toMstoreOrderList?type=3">待收货</a></li>
                    <li <#if type=='4'>class="selected"</#if> ><a href="${ctx}/m/account/mstoreUser/toMstoreOrderList?type=4">待评价</a></li>
                </ul>
            </div>
            <#if list??>
            <#list list as order>
            <div class="borderbox">
                <ul class="prd-list order-prd-list">
                    <li>
                    <div  onclick="goto('${ctx}/m/account/mstoreUser/toMstoreOrderDetail?orderId=${(order.orderId)!}');">
                        <div class="hd">
                            ...
                            <#if type??>
                            	...
                            	<#elseif type=='4'>待评价
                            	<#else>
                            		<#if order.orderStatusCd!=6 && order.orderPayStatusCd==1>
		                            ${DICT('ORDER_STATUS_CD','1')}
		                            <#elseif (order.orderStatusCd!=6) && (order.orderStatusCd==10||order.orderStatusCd==11||order.orderStatusCd==20||order.orderStatusCd==21||order.orderStatusCd==22)>
                        ...
                        <div class="cz">
                        	<#if order.orderPayStatusCd==1 && order.orderStatusCd!=6>
    	...
    	function confirmReceipt(orderId){
            ...
            if (e.index == 0) {
	              ...
	              $.post('${ctx}/m/account/confirmReceipt',{orderId:orderId},function(data){
	    			if(data){
	    				mui.toast('交易完成');
	    				location.href='${ctx}/m/account/mstoreUser/toMstoreOrderDetail?orderId='+orderId;
	    			}else{
	    				mui.toast('操作失败');
	    			}
	    		});
