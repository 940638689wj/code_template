...
<div class="container">
    <ul class="breadcrumb">
        <li><a href="${ctx}/admin/sa/promotion/seckill/list">秒杀</a> <span class="divider">&gt;&gt;</span></li>
        <li class="active"><#if (promotion.promotionId)??>编辑<#else>新增</#if>秒杀信息</li>
    </ul>

    <form id="J_Form" class="form-horizontal" action="${ctx}/admin/sa/promotion/seckill/save" method="post">
        ...
        <div id="edit-div" class="form-content">
                ...
                <div class="control-group">
                    ...
                    <div class="controls control-row-auto">
                        ...
                        <label class="checkbox">
                            <input <#if (promotion.promotionId)??>disabled</#if> <#if isAllUserLevel> checked="checked" </#if> id="allUserLevel" name="allUserLevel" value='1' type="checkbox">
                            全选
                        </label><br>
                        ...
                        <#if userLevelList?? && (userLevelList?size > 0)>
                            ...
                            <#list userLevelList as userLevel>
                                ...
                                <label class="checkbox">
                                    <input <#if (promotion.promotionId)??>disabled</#if> name="userLevel" <#if userLevelSelect> checked='checked' </#if> type="checkbox" value="${(userLevel.userLevelId)!}">
                    ...
                    <div class="controls control-row-auto">
                        <ul class="toolbar">
		                    <li>
		                        ...
		                        <input type="hidden" name="seckillPriceArr" id="seckillPriceArr" value=''>
		                        <input type="hidden" name="userLimitNumArr" id="userLimitNumArr" value=''>
            ...
            <div class="row">
                <div class="control-group controls control-row-auto" id="ConditionDiv">
                    <#if timeList?? && timeList?has_content>
                    	<#list timeList as time>
                    		<#if time_index % 2 == 0>
                    			<div class="ConditionPanel">
                    				<label class="control-label"><s>*</s>有效使用日期：</label>	
                    				<div class="bui-form-group controls control-row-auto" data-rules="{dateRange : [true,'截至时间不能超过起始时间']}">
			                    		<input type="text" class="calendar calendar-time control-text" name="enableStartTime" data-rules="{required:true}" value="${time!}" disabled>
			                    		<span>至</span>
                    		<#else>
	                    				<input type="text" class="calendar calendar-time control-text" name="enableEndTime" data-rules="{required:true}" value="${time!}" disabled>
	                    			</div>
	                    		</div>
                    		</#if>
                    	</#list>
                    <#else>
                    	<div class="ConditionPanel">
                    		...
                    		<div class="bui-form-group controls control-row-auto" data-rules="{dateRange : [true,'截至时间不能超过起始时间']}">
		                    	...
		                    	&nbsp;&nbsp;&nbsp;
	                            <a href="javascript:void(0);" class="addCondition">+添加时间段</a>
	                            <a href="javascript:void(0);" class="delCondition" style="display: none;">删除时间段</a>
                    		</div>
                    	</div>
                    </#if>
                </div>
...
<script type="text/javascript">
    ...
    var productColumns = [
        ...
        {title: '秒杀价', dataIndex: 'price', width: 180,renderer : function(value, obj) {
        	value = value || '';
            return '<input type="text" id="seckillPrice'+obj.productId+'" data-rules="{required:true}"  value="' + value + '" <#if (promotion.promotionId)??>disabled</#if>/>';
        }},
    ...
    function deleteProduct(record){
    	//如果是编辑操作，则该操作禁止
        <#if promotoion?? && promotion?has_content>
        	return;
        </#if>
    ...
    $(function(){
        ...
        var form = new Form.Form({
            ...
            callback: function (data) {
                if (app.ajaxHelper.handleAjaxMsg(data)) {
                    ...
                    window.location.href = "${ctx}/admin/sa/promotion/seckill/list";
                }
            }
        });
        ...
        form.on('beforesubmit', function(){
            ...
            for(var i = 0; i < selectProductIds.length; i++) {
            	...
            	var seckillPrice = $('#seckillPrice'+productId).val().trim();
            	var userLimitNum = $('#userLimitNum'+productId).val().trim();
            	if(seckillPrice == '') {
            		app.showError('请填写商品的秒杀价格!');
            		return false;
            	}
            	
               var reg = /^(([1-9][0-9]{0,15})|(([0]\.\d{1,2}|[1-9][0-9]{0,15}\.\d{1,2})))$/;
	           if(!reg.test(seckillPrice)){
	               app.showError('请填写大于0的合法商品秒杀价格,且整数位不能超过16位!');
            	   return false;
               }
	           ...
	           if(!regNum.test(userLimitNum)){
            	...
            	seckillPriceArr[i] = seckillPrice;
            	userLimitNumArr[i] = userLimitNum;
            }
            
            $('#selectProductIds').val(selectProductIds.join(','));
            $('#seckillPriceArr').val(seckillPriceArr.join(','));
            $('#userLimitNumArr').val(userLimitNumArr.join(','));
            
            var timeIsNull = false;//时间的是否为空,默认时间不为空
            $('input[name="enableStartTime"]').each(function() {
	            if($(this).val() == '') {
	            	app.showError('起始的有效使用时间不能为空!');
	            	timeIsNull = true;
                    return false;//该代码只是脱出each循环，不能终止运行
	            }
            
            });
            
            $('input[name="enableEndTime"]').each(function() {
	            if($(this).val() == '') {
	            	app.showError('结束的有效使用时间不能为空!');
	            	timeIsNull = true;
                    return false;
	            }
