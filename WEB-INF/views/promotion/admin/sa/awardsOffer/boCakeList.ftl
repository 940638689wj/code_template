        ...
        <div class="title-bar-side">
            <div class="search-content">
                <form id="searchForm" class="form-horizontal search-form">
                    ...
                    <input type="hidden" name="Award_Type_Cd" value="3">
...
<script type="text/javascript">
    ...
    tab.on('selectedchange',function (ev) {
        var item = ev.item;
        if(item.get('value')=="0"){
            window.location.href = "${ctx}/admin/sa/promotion/boCake/list";
        }else if(item.get('value')=="1"){
            window.location.href = "${ctx}/admin/sa/promotion/boCake/list?type=expired";
        }else{
            window.location.href = "${ctx}/admin/sa/promotion/boCake/list?type=disable";
        }
    });
    ...
    BUI.use(['common/search','common/page'],function (Search) {
        var columns = [
            ...
            {title:'博饼地址',dataIndex:'id',width:150,renderer:function(value,rowObj){
                //return "/turn/"+rowObj.id+".html";
                var link='<a href="/m/boCake/' + rowObj.id + '.html" target="_blank">/m/boCake/' + rowObj.id + '.html</a>';
                return link;
            }},
