...
<#include "${ctx}/includes/sa/header.ftl"/>
    ...
    <script type="text/javascript">
            ...
            var form = new Form.Form({
                ...
                callback: function (data) {
                    if (app.ajaxHelper.handleAjaxMsg(data)) {
                        if (isNewAdd) { //新增加博饼，保存完成之后，进入下一步：页面配置
                            addNextStep(data);
                        } else { //若为编辑博饼，保存完成之后，保留原页面，提示保存成功
        ...
        function addNextStep(data) {
            app.page.open({
                href: '/admin/sa/promotion/boCake/detail?awardId=' + data.data + '&isAddNew=1'
            });
        }
        ...
        function collectData() {
            ...
            usePoint = usePoint.replace(/^\s\s*/, '').replace(/\s\s*$/, '');
            $("input[name='usePoint']").val(usePoint);
            if (usePoint != "") {
                usePointInt = parseInt(usePoint);
                if (isNaN(usePointInt)) {
                    BUI.Message.Alert("使用积分必须为数值!");
                    return false;
                }
            }

            var recommendPoint = $("input[name='recommendPoint']").val();
            recommendPoint = recommendPoint.replace(/^\s\s*/, '').replace(/\s\s*$/, '');
            $("input[name='recommendPoint']").val(recommendPoint);
            if (recommendPoint != "") {
                recommendPointInt = parseInt(recommendPoint);
                if (isNaN(recommendPointInt)) {
                    BUI.Message.Alert("分享送积分必须为数值!");
                    return false;
                }
            }
        ...
        function assetChange(fileName, fileImageId, hidId) {
            ...
            if (suffix.trim().length <= 0 ||
                    ("jpg" != suffix.trim() && "JPG" != suffix.trim() && "png" != suffix.trim() && "PNG" != suffix.trim())) {
                alert("文件格式错误!");
                return false;
            }
...
<#else>
    <div id="tab">
        <ul class="bui-tab-panel bui-tab nav-tabs bui-tab-panel-hover bui-tab-hover" aria-disabled="false"
            ...
            <li onclick="javascript:void(0);"
                ...
                aria-disabled="false" aria-pressed="false"><span class="bui-tab-item-text">设置博饼规则</span></li>
            <li onclick="javascript:goToTabTwo();" class="bui-tab-panel-item bui-tab-item" aria-disabled="false"
                aria-pressed="false"><span class="bui-tab-item-text">设置博饼页面</span></li>
        </ul>
    </div>
</#if>
    <form id="J_Form" class="form-horizontal" action="edit" method="post">
        ...
        <div class="form-content">
            ...
            <div class="row">
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input value="${(awards.usePoint)!}" name="usePoint" class="input-normal control-text"
                               data-rules="{positiveNum:true}">
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input value="${(awards.oneDayTimes)!}" name="oneDayTimes" class="input-normal control-text"
                               data-rules="{positiveNum:true}">
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input value="${(awards.consumeAmt)!}" name="consumeAmt" class="input-normal control-text"
                               data-rules="{positiveNum:true}">
                    </div>
                </div>
            </div>
        ...
        <#if isNew == false>
            ...
            <#list awardsItemList as item>
                ...
                <#if item_index=0>
                    <div class="row">
                        <div class="control-group">
                            <label class="control-label" style="width:70px;text-align:left;">${(item.seq)?default(temp)}
                                :无中奖</label>
                            <div class="controls">
                ...
                <#else>
                <#--根据序号获取对应的描述-->
                    <#assign boCakeName = item.boCakeAwardsName>
                    ...
                    <div class="row">
                        ...
                        <#if (item.itemPicUrl)?has_content>
                          ...
                          <div class="controls ${defaultHide}">
                              <input type="hidden" id="itemPicUrl_${(temp)!}" name="itemPicUrl_${(temp)!}" value="<#if (assetUrl)?has_content>${assetUrl}</#if>" class="input-normal control-text" >
                             <input style="width: 120px;" id="file_${(temp)!}" name="file" type="file" onChange="assetChange(this.value,'file_${(temp)!}','itemPicUrl_${(temp)!}')">
                        </div>
                        -->
                            <#assign assetUrl = "">
                            <#if (item.itemPicUrl)?has_content>
                            ...
                            <div class="controls" ${defaultHide}>
                                ...
                                <input value="${(item.oneUserMax)?default(0)!}" id="oneUserMax_${temp}"
                            ...
                            <div class="controls" ${defaultHide}>
                                ...
                                <label class="checkbox">
                                    <input class="userFlagClass" id="userFlagBox_${temp}" value=''
                                           type="checkbox"
        ...
        <#else>
            <#list boCakeAwardsNameList as boCakeAwardsNames>
                <#assign boCakeName = boCakeAwardsNames.codeCnName>
                <#assign i = boCakeAwardsNames.displayID>
                <#if i=1>
                <#--系统默认为中奖项不展示-->
                    <div class="row">
                        <div class="control-group">
                            <label class="control-label" style="width:70px;text-align:left;">${i}:无中奖</label>
                            <div class="controls">
                            ...
                            <div class="controls">
                                ...
                                <input value="" name="tip_${i}" data-rules="{required:true}"
                                       class="input-normal control-text" style="width:160px;">
                            </div>
                            <#assign defaultWin = "0">
                            <#assign defaultHide = "style=\"display: none;\"">
                ...
                <#--根据序号获取对应的描述-->
                    <div class="row">
                        <div class="control-group">
                            <label class="control-label" style="width:70px;text-align:left;">${i}
                                :${boCakeName!}</label>
                            <div class="controls">
                                <label class="control-label" style="width: 80px;"><s>*</s>奖项内容：</label>
                                <input value="" name="name_${i}" data-rules="{required:true}"
                                       class="input-normal control-text" style="width:160px;">
                            </div>
                            <div class="controls">
                                <label class="control-label" style="width: 80px;"><s>*</s>奖项提示：</label>
                            ...
                            <div class="controls">
                                <label class="checkbox">
                                    <input class="winningFlagClass" id="winningFlagBox_${i}" value='1'
                                           type="checkbox"
                                           <#if defaultWin == "1">checked="checked"</#if>>是否中奖</label>
                        ...
                        <#assign assetUrl = "">
                          <div class="controls ${defaultHide}">
                              <input type="hidden" id="itemPicUrl_${(i)!}" name="itemPicUrl_${(i)!}" value="<#if (assetUrl)?has_content>${assetUrl}</#if>" class="input-normal control-text" >
                             <input style="width: 120px;" id="file_${(i)!}" name="file" type="file" onChange="assetChange(this.value,'file_${(i)!}','itemPicUrl_${(i)!}')">
                         </div>
                        -->
                            <#assign assetUrl = "">
                            <#--<div class="controls" ${defaultHide}>-->
                            ...
                            <div class="controls" ${defaultHide}>
                                <label class="control-label" style="width: 80px;"><s>*</s>奖项个数：</label>
                                <input value="" id="count_${i}" name="count_${i}"
                                       class="input-normal control-text" style="width:60px;">
                            </div>
                            <div class="controls" ${defaultHide}>
                                <label class="control-label" style="width: 80px;"><s>*</s>奖项概率：</label>
                                <input value="" id="probability_${i}" name="probability_${i}"
                                       class="input-normal control-text" style="width:60px;">%
                            </div>
                            <div class="controls" ${defaultHide}>
                                <label class="control-label" style="width: 100px;"><s>*</s>单用户最多：</label>
                            ...
                            <div class="controls" ${defaultHide}>
                                ...
                                <input type="hidden" id="usePointFlagHidden_${i}" name="usePointFlag_${i}"
                                       value="0">
                                <input type="text" id="usePointAmount_${i}" class="control-text"
                                       name="usePointAmount_${i}" style="display: none;width:60px;" value="">
                            </div>
                            <div class="controls" ${defaultHide}>
                                <label class="checkbox"><input class="couponFlagClass" id="couponFlagBox_${i}"
                                                               value=''
                            ...
                            <div class="controls" ${defaultHide}>
                                <label class="checkbox"><input class="userFlagClass" id="userFlagBox_${i}" value=''
                                                               type="checkbox">指定中奖人</label>
                                <input type="hidden" id="userFlagHidden_${i}" name="userFlag_${i}" value="0">
                                <input type="button" class="userChoiceBtn" id="userChoiceBtn_${i}"
                                       name="userChoiceBtn"
