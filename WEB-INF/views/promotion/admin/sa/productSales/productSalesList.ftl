...
<script type="text/javascript">
    ...
    tab.on('selectedchange',function (ev) {
        var item = ev.item;
        if(item.get('value')=="0"){
            window.location.href = "/admin/sa/promotion/productSales/list?type=${type!}";
        }else if(item.get('value')=="1"){
            window.location.href = "/admin/sa/promotion/productSales/list?type=${type!}&statusCd=1";
        }else{
            window.location.href = "/admin/sa/promotion/productSales/list?type=${type!}&statusCd=0";
        }
    });
    ...
    BUI.use(['common/search','common/page'],function (Search) {
        var columns = [
            ...
            {title:'优惠类型',dataIndex:'promotionTypeCd', width:100, renderer: function(val, rowObj){
                var offerType = '${Static["cn.yr.chile.promotion.constant.PromotionType"].DETAIL_ITEM_DISCOUNT_TYPE.getFriendlyType()}';
                if(${Static["cn.yr.chile.promotion.constant.PromotionType"].DETAIL_ITEM_REDUCE_TYPE.getType()} == val){
                    offerType = '${Static["cn.yr.chile.promotion.constant.PromotionType"].DETAIL_ITEM_REDUCE_TYPE.getFriendlyType()}';
                } else if (${Static["cn.yr.chile.promotion.constant.PromotionType"].DETAIL_ITEM_SPIKE_TYPE.getType()} == val){
                    offerType = '${Static["cn.yr.chile.promotion.constant.PromotionType"].DETAIL_ITEM_SPIKE_TYPE.getFriendlyType()}';
                } 
                return offerType;
            }},
            ...
            {title:'操作',dataIndex:'',width:100,renderer : function(value,rowObj){
                var str = Search.createLink({
                    text: '编辑',
                    href: '${ctx}/admin/sa/promotion/productSales/edit?promotionId=' + rowObj.promotionId + '&type=${type!}'
                });
<@securityAuthorize ifAnyGranted="delete">
        ...
        store = Search.createStore('/admin/sa/promotion/productSales/grid_json?type=${type!}&status=${status!}',{pageSize:15});
    ...
    function addFunction(){
        app.page.open({
            href:'${ctx}/admin/sa/promotion/productSales/edit?type=${type!}'
        });
    }
    function del(id) {
