...
<div class="container">
    ...
    <div class="content-top">
        <form id="searchForm" class="form-horizontal search-form" action="${ctx}/admin/sa/preOrderPriceAudit/list">
            <div class="row">
				...
				<#if pageDTO?has_content&&(pageDTO.content)?has_content>
					<#list pageDTO.content as one>
						<li id="${(one.id)!}">
							...
							<div class="con">订单号:<a href="${ctx}/admin/sa/order/toOrderDetail?orderId=${(one.orderId)!}" target="_blank">${(one.orderNumber)!}</a>
...
<#macro pager pageDTO steps=3>
    ...
    <script>
        ...
        function goPage(pageNo){
            window.location.href ="${ctx}/admin/sa/preOrderPriceAudit/list?pageNo="+pageNo;
        }
...
<script type="text/javascript">
    ...
    tab.on('selectedchange',function (ev) {
        var item = ev.item;
        if(item.get('value')=="0"){
            window.location.href = "${ctx}/admin/sa/preOrderPriceAudit/list";
        }
        if(item.get('value')=="1"){
            window.location.href = "${ctx}/admin/sa/preOrderPriceAudit/doneList";
        }
    });
    ...
    function pass(auditId){
	    ...
	    BUI.Message.Confirm('确定要同意调价吗?',function(){
	        $.ajax({
	            url : '${ctx}/admin/sa/preOrderPriceAudit/pass/'+auditId,
	            dataType : 'json',
	            type: 'POST',
	...
	function refuse(auditId){
	    ...
	    BUI.Message.Confirm('确定要拒绝调价吗?',function(){
	        $.ajax({
	            url : '${ctx}/admin/sa/preOrderPriceAudit/refuse/'+auditId,
	            dataType : 'json',
	            type: 'POST',
	            success : function(data){
	                if(data.result == "success"){
	...
	function passAll(){
	    ...
	    BUI.Message.Confirm('确定要批量同意调价吗?',function(){
	        $.ajax({
	            url : '${ctx}/admin/sa/preOrderPriceAudit/passAll/',
	            dataType : 'json',
	            type: 'POST',
	...
	function refuseAll(){
	    ...
	    BUI.Message.Confirm('确定要批量拒绝调价吗?',function(){
	        $.ajax({
	            url : '${ctx}/admin/sa/preOrderPriceAudit/refuseAll',
	            dataType : 'json',
	            type: 'POST',
	            data : {id : selectedCustomerIds},
	            success : function(data){
