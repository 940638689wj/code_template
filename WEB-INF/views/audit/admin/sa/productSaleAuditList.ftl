...
<div class="container">
    ...
    <div class="content-top">
        <ul class="toolbar">
            <li style="margin-left: 7px;"><label class="checkbox" for="chk_all"><input type="checkbox"
                                                                                       id="chk_all"/>全选</label></li>
            <li>
            ...
            <#if pageDTO?has_content&&(pageDTO.content)?has_content>
                <#list pageDTO.content as one>
                    <li id="${(one.id)!}">
                        ...
                        <div class="con">
                            商品: ${one.productName!} 的状态，
                            <#if one??&&one?has_content&&one.newValue=='1'>
                                从下架状态更改为上架状态,
                            <#elseif one??&&one?has_content&&one.newValue=='2'>
                                从上架状态更改为下架状态,
                            </#if>
...
<script>
    ...
    function goPage(pageNo) {
        window.location.href = "${ctx}/admin/sa/productAudit/saleAuditList?pageNo=" + pageNo;
    }
...
<script type="text/javascript">
    ...
    function pass(auditId) {
        ...
        BUI.Message.Confirm('确定要同意此商品审核吗?', function () {
            $.ajax({
                url: '${ctx}/admin/sa/productAudit/passStatusChange/' + auditId,
                dataType: 'json',
                type: 'POST',
    ...
    function refuse(auditId) {
        ...
        BUI.Message.Confirm('确定要拒绝此商品审核吗?', function () {
            $.ajax({
    ...
    function passAll() {
        ...
        BUI.Message.Confirm('确定要批量商品审核吗?', function () {
            $.ajax({
                url: '${ctx}/admin/sa/productAudit/passStatusChangeAll/',
                dataType: 'json',
                type: 'POST',
                data: {id: selectedCustomerIds},
                success: function (data) {
                    if (data.result == "success") {
                        app.showSuccess("已同意了商品审核申请!");
