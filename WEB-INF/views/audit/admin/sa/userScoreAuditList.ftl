...
<div class="container">
    ...
    <div class="content-top">
        <form id="searchForm" class="form-horizontal search-form" action="${ctx}/admin/sa/userScoreAudit/list">
            <div class="row">
                <div class="search-bar">
                    <input id="applySrcUserName" name="applySrcUserName" class="control-text" type="text"
                           placeholder="用户名"/>
            ...
            <#if pageDTO?has_content&&(pageDTO.content)?has_content>
                <#list pageDTO.content as one>
                    <li id="${(one.id)!}">
                        ...
                        <div class="con">用户:<a href="${ctx}/admin/sa/user/toUserDetailIndex/${(one.applySrcUserId)!}"
                                               target="_blank">${(one.applySrcUserName)!}</a>，
                            调整积分，调整积分为:<#if one.newValue??&&one.newValue?has_content>${(one.newValue)!}
                                                    <#elseif one.oldValue??&&one.oldValue?has_content>-${(one.oldValue)!}</#if><br>
...
<script type="text/javascript">
    ...
    function goPage(pageNo) {
        window.location.href = "${ctx}/admin/sa/userScoreAudit/list?pageNo=" + pageNo;
    }
...
<script type="text/javascript">
    ...
    tab.on('selectedchange', function (ev) {
        var item = ev.item;
        if (item.get('value') == "0") {
            window.location.href = "${ctx}/admin/sa/userScoreAudit/list";
        }
        if (item.get('value') == "1") {
            window.location.href = "${ctx}/admin/sa/userScoreAudit/doneList";
        }
    });
    ...
    function pass(auditId) {
        ...
        BUI.Message.Confirm('确定要同意该申请吗?', function () {
            $.ajax({
                url: '${ctx}/admin/sa/userScoreAudit/pass/' + auditId,
                dataType: 'json',
                type: 'POST',
                success: function (data) {
                    if (data.result == "success") {
                        app.showSuccess("成功通过该申请!");
                        var id = "#" + auditId;
                        $(id).css("display", "none")
                    } else {
    ...
    function refuse(auditId) {
        ...
        BUI.Message.Confirm('确定要拒绝该申请吗?', function () {
            $.ajax({
                url: '${ctx}/admin/sa/userScoreAudit/refuse/' + auditId,
                dataType: 'json',
                type: 'POST',
                success: function (data) {
                    if (data.result == "success") {
                        app.showSuccess("成功拒绝该申请!");
                        var id = "#" + auditId;
                        $(id).css("display", "none")
                    } else {
                        app.showError("审核失败!");
    ...
    function passAll() {
        ...
        $(".check-list input[type='checkbox']").each(function () {
            if ($(this).attr("checked")) {
                var id = $(this).attr("value");//获取value的值
                selectedCustomerIds.push(id);//放入到数组中
            }
        });
        ...
        BUI.Message.Confirm('确定要批量同意申请吗?', function () {
            $.ajax({
                url: '${ctx}/admin/sa/userScoreAudit/passAll',
                dataType: 'json',
                type: 'POST',
    ...
    function refuseAll() {
        ...
        BUI.Message.Confirm('确定要批量拒绝申请?', function () {
            $.ajax({
                url: '${ctx}/admin/sa/userScoreAudit/refuseAll',
                dataType: 'json',
                type: 'POST',
                data: {id: selectedCustomerIds},
                success: function (data) {
