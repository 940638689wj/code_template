...
<div class="container">
    ...
    <div class="content-top">
        <form id="searchForm" class="form-horizontal search-form" action="${ctx}/admin/sa/returnAudit/list">
            <div class="row">
		...
		<#if pageDTO?has_content&&(pageDTO.content)?has_content>
			<#list pageDTO.content as one>
				<li id="${(one.id)!}">
					...
					<div class="con">订单号:
						<a href="${ctx}/admin/sa/customerService/toOrderReturnDetail?returnOrderNumber=${(one.returnOrderNumber)!}" target="_blank">
						${(one.orderNumber)!}</a>，
						申请退款<br>
...
<script>
    ...
    function goPage(pageNo){
        window.location.href ="${ctx}/admin/sa/returnAudit/list?pageNo="+pageNo;
    }
...
<script type="text/javascript">
    ...
    function pass(auditId){
	    ...
	    BUI.Message.Confirm('确定要同意该申请吗?',function(){
	        $.ajax({
	            url : '${ctx}/admin/sa/returnAudit/pass/'+auditId,
	            dataType : 'json',
	            type: 'POST',
	...
	function refuse(auditId){
	    ...
	    BUI.Message.Confirm('确定要拒绝该申请吗?',function(){
	        $.ajax({
	            url : '${ctx}/admin/sa/returnAudit/refuse/'+auditId,
	            dataType : 'json',
	            type: 'POST',
	            success : function(data){
	                if(data.result == "success"){
	...
	function passAll(){
	    ...
	    BUI.Message.Confirm('确定要批量同意申请吗?',function(){
	        $.ajax({
	            url : '${ctx}/admin/sa/returnAudit/passAll',
	            dataType : 'json',
	            type: 'POST',
	...
	function refuseAll(){
	    ...
	    BUI.Message.Confirm('确定要批量拒绝申请?',function(){
	        $.ajax({
	            url : '${ctx}/admin/sa/returnAudit/refuseAll',
	            dataType : 'json',
	            type: 'POST',
	            data : {id : selectedCustomerIds},
	            success : function(data){
