...
<script type="text/javascript">
    ...
    BUI.use(['common/search', 'common/page'], function (Search) {
        var columns = [
            {
                title: '审核对象', dataIndex: 'applySrcUserName', width: 200, renderer: function (value, rowObj) {
                var str = "";
                str = "<a href='javascript:getUser(\"" + rowObj.applySrcUserId + "\");'>" + value + "</a>";
                return str;
            }
            },
            {
                title: '审核内容', dataIndex: '', width: 200, renderer: function (value, rowObj) {
                if(rowObj.newValue!=null)
                    return "充值积分" + rowObj.newValue;
                else if (rowObj.oldValue!=null)
                    return "充值积分-" + rowObj.oldValue;
            }
            },
            {
        ...
        var store = Search.createStore('${ctx}/admin/sa/userScoreAudit/grid_json', {pageSize: 15});
    ...
    function getUser(obj) {
        window.location.href = "${ctx}/admin/sa/user/toUserDetailIndex/" + obj;
    }
