...
<div class="container">
    ...
    <div class="content-top">
        <form id="searchForm" class="form-horizontal search-form" action="${ctx}/admin/sa/userPriceAudit/list">
            <div class="row">
                ...
                <#list pageDTO.content as one>
                    <li id="${(one.id)!}">
                        ...
                        <div class="con">用户:<a href="${ctx}/admin/sa/user/toUserDetailIndex/${(one.applySrcUserId)!}"
                            ...
                            申请充值，充值金额为:<#if one.newPrice??&&one.newPrice?has_content>￥${(one.newPrice)!}
                                                    <#elseif one.oldPrice??&&one.oldPrice?has_content>-￥${(one.oldPrice)!}</#if><br>
...
<script>
    ...
    function goPage(pageNo) {
        window.location.href = "${ctx}/admin/sa/userPriceAudit/list?pageNo=" + pageNo;
    }
...
<script type="text/javascript">
    ...
    tab.on('selectedchange', function (ev) {
        var item = ev.item;
        if (item.get('value') == "0") {
            window.location.href = "${ctx}/admin/sa/userPriceAudit/list";
        }
        if (item.get('value') == "1") {
            window.location.href = "${ctx}/admin/sa/userPriceAudit/doneList";
        }
    });
    ...
    function pass(auditId) {
        ...
        BUI.Message.Confirm('确定要同意该申请吗?', function () {
            $.ajax({
                url: '${ctx}/admin/sa/userPriceAudit/pass/' + auditId,
                dataType: 'json',
                type: 'POST',
    ...
    function refuse(auditId) {
        ...
        BUI.Message.Confirm('确定要拒绝该申请吗?', function () {
            $.ajax({
                url: '${ctx}/admin/sa/userPriceAudit/refuse/' + auditId,
                dataType: 'json',
                type: 'POST',
                success: function (data) {
                    if (data.result == "success") {
    ...
    function passAll() {
        ...
        BUI.Message.Confirm('确定要批量同意申请吗?', function () {
            $.ajax({
                url: '${ctx}/admin/sa/userPriceAudit/passAll',
                dataType: 'json',
                type: 'POST',
    ...
    function refuseAll() {
        ...
        BUI.Message.Confirm('确定要批量拒绝申请?', function () {
            $.ajax({
                url: '${ctx}/admin/sa/userPriceAudit/refuseAll',
                dataType: 'json',
                type: 'POST',
                data: {id: selectedCustomerIds},
                success: function (data) {
