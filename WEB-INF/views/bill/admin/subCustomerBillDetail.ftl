            ...
            <div class="row">
                <div class="control-group control-row-auto">
                    <div class="controls ml0">
                        ...
                        <div class="pull-left">
                        	<input type="text" id="loginName" name="loginName" class="control-text" placeholder="上级会员账号">
...
<script type="text/javascript">
   //导出
	function generateAllReport(){
		...
		location.href ="${ctx}/admin/sa/subCustomerBillDetail/exportCustomerBillDetail?"+params;
...
<script type="text/javascript">
   ...
   tab.setSelected(tab.getItemAt(0));
	...
	var columns = [
			...
			{title : '会员',dataIndex :'phone',width:150, renderer:function(value,rowObj){
				return value;
			}},
			{title : '订单金额',dataIndex :'orderAmt',width:150, renderer:function(value,rowObj){
				return value;
			}},
			{title : '商品数量',dataIndex :'quantity',width:150, renderer:function(value,rowObj){
				return value;
			}},
			{title : '下单时间',dataIndex :'createTime',width:150, renderer: BUI.Grid.Format.datetimeRenderer},
			{title : '确认收货时间',dataIndex :'completeTime',width:150, renderer: BUI.Grid.Format.datetimeRenderer}
			
		];
		
		store = Search.createStore('${ctx}/admin/sa/subCustomerBillDetail/grid_json',{pageSize:20});
    ...
    function search_sum(){
            ...
            if(!loginName){
            	BUI.Message.Alert('请输入上级会员账号','error');
            	return;
            }
            search.load();
            $.ajax({
                url:'${ctx}/admin/sa/subCustomerBillDetail/getTotal',
                dataType : 'json',
                method : 'get',
                data : {beginTime:beginTime,endTime:endTime,loginName:loginName},
                success : function(data){
                    $("#orderTotal").html(data.orderTotal == null ? 0 : data.orderTotal) ,
                    $("#productTotal").html(data.quantity == null ? 0 : data.quantity),
                    $("#totalAmt").html('￥'+(data.orderAmt == null ? 0 : data.orderAmt))          	                         
                }
            })
        };		
