...
<script type="text/javascript">
	...
	function generateAllReport(){
		...
		location.href ="${ctx}/admin/sa/subCustomerTotalBill/exportSubCustomerTotalBill?"+params;
...
<script type="text/javascript">
    ...
    var tab = new Tab.Tab({
        ...
        autoRender: true,
        children:[
        	{text:'下级客户总账单',value:'0'}
        ]
    });
    tab.setSelected(tab.getItemAt(0));
	BUI.use(['common/search','common/page'],function (Search) {
	var columns = [
			{title : '会员账号',dataIndex :'phone',width:200, renderer: function(value,rowObj){
				if(value==null){
					value='';
				}
					return value;
			}},
			{title : '自身消费订单',dataIndex :'mOrderNumber',width:200, renderer:function(value,rowObj){
				if(value==null){
					value='';
				}
				return value;
			}},
			{title : '自身消费总金额',dataIndex :'mTotalAmt',width:200, renderer:function(value,rowObj){
				if(value==null){
					value=0;
				}
				return value;
			}},
			{title : '下级销售总订单',dataIndex :'subOrderTotal',width:200, renderer:function(value,rowObj){
				if(value==null){
					value=0;
				}
				return value;
			}},
			{title : '下级销售总金额',dataIndex :'subTotalAmt',width:200, renderer:function(value,rowObj){
				if(value==null){
					value=0;
				}
				return value;
		...
		var store = Search.createStore('${ctx}/admin/sa/subCustomerTotalBill/grid_json',{pageSize:20});
